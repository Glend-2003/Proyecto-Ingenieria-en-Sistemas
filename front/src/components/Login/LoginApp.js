...
function LoginApp({ initialPage = "home" }) {
  ...
  const { showSidebar, handleShowSidebar, addToCart, updateUserStatus } =
    useAppContext();
  ...
  const validateFields = () => {
    ...
    if (!loginData.correoUsuario) {
      ...
      isValid = false;
      showAlert(
        `Incluye un signo '@' en la dirección de correo electrónico. La dirección "${loginData.correoUsuario}" no incluye el signo '@'.`,
        "warning"
      );
    }
    ...
    if (name === "correoUsuario" && value && !validateEmail(value)) {
      ...
      if (value.length > 3) {
        ...
        const timer = setTimeout(() => {
          showAlert(
            `Incluye un signo '@' en la dirección de correo electrónico. La dirección "${value}" no incluye el signo '@'.`,
            "warning"
          );
        }, 1000);
    ...
    axios
      ...
      .then((response) => {
        ...
        if (response.data.token) {
          ...
          showAlert(
            `Bienvenido ${response.data.nombreUsuario} a Carnicería La Bendición`,
            "success"
          );
  ...
  const handleForgotPassword = async (e) => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:8080/usuario/verificarCambioContrasena",
        {
      ...
      if (response.status === 200) {
        showAlert(
          "Código enviado con éxito a tu correo electrónico",
          "success"
        );
        ...
        setTimeout(() => {
          navigate("/ResetPassword", {
            state: { correoUsuario: loginData.correoUsuario },
          });
        }, 2000);
      }
    } catch (error) {
      if (!error.response) {
        ...
        showAlert(
          "Error al enviar el código: " +
            (error.response?.data?.message || "Correo inexistente"),
          "error"
        );
      }
    } finally {
  ...
  return (
    <div className="page-container">
      ...
      >
        <CentralizedAlert severity={alert.severity} onClose={handleCloseAlert}>
          {alert.message}
        </CentralizedAlert>
      </div>
